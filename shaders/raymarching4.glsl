...
float sdf(vec3 p) {
    ...
    vec3 p2 = vec3(p1.x -snoise(vec2(u_time/4., .4)), p1.y-snoise(vec2(u_time/4., .9)), p1.z-snoise(vec2(u_time/4., .9)));
    vec3 r = opRep(p1, vec3(.35));
    float box = sdBoxFrame(r, vec3(.03), .005);
    // float box2 = sdBox(p2, vec3(.2));
    // box = max(box, -box2);
    float sphere = sdSphere(p1, u_time/3.);
    box = smin(sphere, box, .1);
