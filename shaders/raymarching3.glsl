...
float sdf(vec3 p) {
    ...
    vec3 p2 = vec3(p1.x -snoise(vec2(u_time/4., .4)), p1.y-snoise(vec2(u_time/4., .9)), p1.z-snoise(vec2(u_time/4., .1)));
    vec3 r = opRep(p1, vec3(.35));
    float box = sdBox(r, vec3(.03));
    // float box2 = sdBox(p2, vec3(.2));
    // box = max(box, -box2);
    float sphere = sdSphere(p2, 0.2);
    box = smax(-sphere, box, .7);
...
void main() {
    ...
    vec3 bg = mix(vec3(1.0, 0.902, 0.0), vec3(0.), dist);
